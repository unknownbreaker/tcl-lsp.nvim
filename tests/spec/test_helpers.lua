...
function M.create_vim_mock()
  local mock = {
    ...
    fs = {
      ...
      find = function(patterns, opts)
        -- Mock implementation - would normally search filesystem
        return {}
      end,
    },
    ...
    fn = {
      ...
      tempname = function()
        return "/tmp/nvim_test_" .. math.random(10000, 99999)
      end,
      mkdir = function()
        return 1
      end,
...
function M.create_logger_mock()
  ...
  return {
    debug = function(msg)
      table.insert(log_messages, { level = "debug", msg = msg })
    end,
    info = function(msg)
      table.insert(log_messages, { level = "info", msg = msg })
    end,
    warn = function(msg)
      table.insert(log_messages, { level = "warn", msg = msg })
    end,
    error = function(msg)
      table.insert(log_messages, { level = "error", msg = msg })
    end,
...
function M.create_config_mock(config_values)
  ...
  return {
    ...
    update = function(new_config)
      config_values = vim.tbl_deep_extend("force", config_values, new_config)
    end,
